...
import { useCouponCreate } from '@/hooks/api/coupon'
...
const CouponAddPageMain = () => {
    const {register,handleSubmit,formState:{errors},watch} = useForm<IcouponBase>({
    ...
    const createCouponMutation = useCouponCreate()
    const {toast} = useToast() 
    const discount_typeWatch = watch('discount_type')
    const onSubmit = (data:IcouponBase) => {
        ...
        createCouponMutation.mutate(newData)
    }
    return (
        <div>
                ...
                <form className='bg-white  p-4 space-y-6' onSubmit={handleSubmit(onSubmit)}>
                    <div className='grid grid-cols-[100px_auto] gap-3 '>
                        ...
                        <div className='w-full'>
                            <input type="text" {...register('code')} className={` text-sm px-3 outline-0 py-1 border w-full focus:outline-1 focus:outline-blue-500`} />
...
export default CouponAddPageMain