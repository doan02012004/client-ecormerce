...
const VariantsProductEdit = () => {

    const { register, control, setValue, formState: { errors } } = useFormContext<IproductFormEdit>()
            ...
            options[0].values.map((value, index) => {
                const indexModel = models.findIndex((model) => {
                    if (model.name == `${value.label}`) {
                        return true
                    } else if (model.tiers_index.length == 1 && model.tiers_index[0] === index) {
                        return true
                    } else {
                        return false
                    }
                ...
                if (indexModel >= 0) {
                    variants.push({ ...models[indexModel], is_default:false, name: `${value.label}`, tiers_index: [index], image: value.image, discount: 0 })
                } else {
                    variants.push({
                        id: genarateId(),
                        name: `${value.label}`,
        ...
        if (options.length === 2) {
            options[0].values.map((value1, index1) => {
                options[1].values.map((value2, index2) => {
                    ...
                    if (indexModel >= 0) {
                        variants.push({ ...models[indexModel], is_default:false, name: `${value1.label}, ${value2.label}`, image: value1.image, tiers_index: [index1, index2] })
                    } else {
                        variants.push({
                            id: genarateId(),
...
export default VariantsProductEdit