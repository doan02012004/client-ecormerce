...
import { shipEditFormSchema } from '@/schemas/ship'
import {  IshipFormEdit } from '@/types/ship'
import { zodResolver } from '@hookform/resolvers/zod'
import React from 'react'
import { useFieldArray, useForm } from 'react-hook-form'
import { Plus } from 'lucide-react'
import { genarateId } from '@/utils/main'
import { useShipUpdate } from '@/hooks/api/ship'
import ShipItemFormEdit from './ShipItemFormEdit'
import ShipItemEditDefault from './ShipItemEditDefault'
import { useToast } from '@/hooks/use-toast'

const ShipMainEditAdmin = ({ship}:{ship:IshipFormEdit}) => {
    const { register, handleSubmit, formState: { errors,isDirty }, watch, setValue, control } = useForm<IshipFormEdit>({
        resolver: zodResolver(shipEditFormSchema),
        mode:'onChange',
        defaultValues: ship
    })
    const shipUpdateMutation = useShipUpdate()
    ...
    const onSubmit = (data:IshipFormEdit) => {
        if(!isDirty){
            return toast({
        ...
        console.log(data)
        shipUpdateMutation.mutate(data)
    }
                    ...
                    <div className='grid grid-cols-[100px_auto] gap-3 '>
                        ...
                        <div className='space-y-4'>
                            {items.map((item, index) => (
                                <ShipItemFormEdit key={item.id} items={items} errors={errors} register={register} remove={remove} index={index} onChangeMaxWeight={onChangeMaxWeight} />
                            ))}
                            {errors.items && errors.items.root && (<p className='text-red-500 text-xs'>{errors.items.root.message}</p>)}
                            <ShipItemEditDefault  register={register} errors={errors} />
...
export default ShipMainEditAdmin